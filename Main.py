...
def user_loop(user_list, number_tweets, trim, previous=False):
        ...
        for user in user_list:
            try:
                ...
                if previous is True:
                    ...
                    Sent.replies(api, user, number_elements, replies_data, previous=True)
                else:
                    ...
                    Sent.replies(api, user, number_elements, replies_data, previous=False)
            except IOError:
                if previous is True:
                    ...
                    print("No previous replies found for " + user + ", downloading last " + str(number_elements) +
                          " replies")
                    replies_data = []
                    Sent.replies(api, user, number_elements, replies_data)
        ...
        for user in user_list:
            try:
                ...
                if previous is True:
                    ...
                    Sent.tweets(api, user, number_elements, tweets_data, trim, previous=True)
                else:
                    ...
                    Sent.tweets(api, user, number_elements, tweets_data, trim, previous=False)
            except IOError:
                if previous is True:
                    print("Download some data first with tweets_replies_last_loop function")
                else:
